    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <motion.div
          ...
          exit={{ opacity: 0 }}
          onClick={onClose}
          className="absolute inset-0 bg-black/80 dark:bg-black/80 backdrop-blur-sm"
        />
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-md max-h-[80vh] rounded-3xl border border-slate-200/50 dark:border-white/10 bg-gradient-to-br from-white dark:from-slate-900/95 via-slate-50 dark:via-slate-900/90 to-white dark:to-slate-900/95 backdrop-blur-xl shadow-elegant-lg overflow-hidden flex flex-col"
        >
          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b border-slate-200/50 dark:border-white/10">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-white/10 transition-colors text-slate-600 dark:text-white/70"
            >
              <XIcon />
            </button>
          </div>
          ...
          <div className="px-6 py-4 border-b border-slate-200/50 dark:border-white/10">
            <input
              ...
              className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-300 dark:border-white/10 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50"
            />
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="size-16 rounded-full bg-slate-200 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 flex items-center justify-center mb-4">
                ...
                {users.map((user) => (
                  ...
                  >
                    <Link
                      href={`/${user.handle}`}
                      onClick={onClose}
                      className="flex items-center gap-3 rounded-xl p-3 transition-all duration-200 hover:bg-slate-100 dark:hover:bg-white/5"
                    >
                      <div className="relative size-12 flex-shrink-0">
                        <Image
                          ...
                          fill
                          className="rounded-full object-cover ring-2 ring-slate-300 dark:ring-slate-800 group-hover:ring-primary-500/50 transition-all"
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-semibold text-slate-900 dark:text-white truncate">{user.displayName}</p>
                          {user.isVerified && (
