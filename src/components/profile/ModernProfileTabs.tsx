...
export default function ModernProfileTabs({ username, isOwnProfile }: ModernProfileTabsProps) {
    ...
    if (error) {
      return (
        <div className="text-center py-16 px-6">
          ...
          <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 dark:text-gray-100 mb-3">Error al cargar contenido</h3>
          <p className="text-gray-600 dark:text-gray-400 dark:text-gray-500 mb-6">{error}</p>
          <button
        ...
        {hasMore && (
          <div className="text-center mt-6">
            <button
              ...
              disabled={loading}
              className="px-6 py-3 bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 dark:text-gray-300 rounded-lg hover:from-gray-200 hover:to-gray-300 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg transform hover:-translate-y-0.5 font-medium text-sm"
            >
              {loading ? (
  ...
  return (
    <div className="relative">
      ...
      <div className="relative bg-white dark:bg-gray-900 dark:bg-gray-900 dark:bg-gray-900/95 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 dark:border-gray-700/50 overflow-hidden">
        {/* Pestañas mejoradas */}
        <div className="border-b border-gray-200 dark:border-gray-700 dark:border-gray-700 dark:border-gray-700/50">
          <div className="flex">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                ...
                className={`flex-1 flex items-center justify-center gap-2 px-4 py-4 text-sm font-medium transition-all duration-300 relative ${activeTab === tab.id
                  ...
                  : 'text-gray-600 dark:text-gray-400 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:text-gray-100 dark:text-gray-100 dark:text-gray-100 hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-800 dark:bg-gray-800 dark:bg-gray-800/50'
                  }`}
              >
                {activeTab === tab.id && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 to-purple-500"></div>
                )}
                <div className={`transition-colors duration-300 ${activeTab === tab.id ? 'text-blue-600' : 'text-gray-500 dark:text-gray-400 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400'}`}>
                  {tab.icon}
                </div>
                ...
                <span
                  ...
                  className={`px-2 py-1 rounded-full text-xs font-bold transition-all duration-300 cursor-pointer hover:scale-105 ${activeTab === tab.id
                    ...
                    : 'bg-gray-100 dark:bg-gray-700 dark:bg-gray-700 dark:bg-gray-800 text-gray-600 dark:text-gray-400 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600 dark:bg-gray-600 dark:bg-gray-700'
...
function ModernEmptyState({ tabType, isOwnProfile, router }: { tabType: TabType; isOwnProfile: boolean; router: AppRouterInstance }) {
  ...
  return (
    <div className="text-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 mb-2">{message.title}</h3>
      <p className="text-gray-600 dark:text-gray-400 dark:text-gray-500 mb-6 max-w-sm mx-auto leading-relaxed text-sm">{message.subtitle}</p>
      {isOwnProfile && (
      ...
      {safeContent.map((item) => {
        ...
        return (
          <div
            ...
            className="aspect-[9/16] rounded-2xl lg:rounded-3xl overflow-hidden bg-gray-100 dark:bg-gray-700 dark:bg-gray-700 dark:bg-gray-800 relative group cursor-pointer shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
          >
            <div className="absolute inset-0">
              {type === 'posts' ? (
                <Image
            ...
            {type === 'reels' && (
              <>
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="w-12 h-12 bg-white dark:bg-gray-900 dark:bg-gray-900 dark:bg-gray-900/90 rounded-full flex items-center justify-center">
                    <svg className="w-6 h-6 text-gray-900 dark:text-gray-100 dark:text-gray-100 ml-1" fill="currentColor" viewBox="0 0 20 20">
