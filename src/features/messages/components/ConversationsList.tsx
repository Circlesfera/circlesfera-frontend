  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      if (response && response.data) {
        ...
        logger.warn('No conversations in response', { userId: user?.id });
        setConversations([]);
      }
          ...
          <ul className="p-2 space-y-1">
            <AnimatePresence>
              {filteredConversations.map((conversation) => (
                ...
                >
                  <button
                    className={`group w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 ${selectedId === conversation.id
                      ? 'bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-400 shadow-sm dark:shadow-gray-900/50'
                  ...
                  >
                    ...
                    <div className="relative flex-shrink-0">
                      <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full overflow-hidden ring-2 ${selectedId === conversation.id ? 'ring-blue-400' : 'ring-gray-200 group-hover:ring-gray-300 dark:ring-gray-600'
                        } transition-all`}>
