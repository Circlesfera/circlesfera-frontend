...
import { PostImage } from '@/shared/components/Image/Image'
...
export function PostCardMedia({ post }: PostCardMediaProps) {
  ...
  const getAspectRatioClass = () => {
    return 'aspect-square' // Default aspect ratio
  }
  ...
  const handleNextImage = () => {
    if (post.content.media) {
      setCurrentImageIndex((prev) => Math.min(post.content.media!.length - 1, prev + 1))
    }
  }
  ...
  switch (post.type) {
    case 'image':
      ...
      if (post.content.media && post.content.media.length > 1) {
        const totalImages = post.content.media.length
        ...
        return (
          ...
          >
            <div className={`relative ${aspectRatioClass} bg-black`}>
              {/* Imagen actual */}
              <PostImage
                src={post.content.media[currentImageIndex]?.url || ''}
    ...
    case 'video':
      return (
        ...
        >
          ...
          {/* Duración del video */}
          {post.content.media?.[0]?.duration && !isVideoPlaying && (
            <div
              className="absolute bottom-2 right-2 bg-black/70 text-white text-xs px-2 py-1 rounded backdrop-blur-sm"
              aria-label={`Duración: ${formatDuration(post.content.media?.[0].duration)}`}
            >
              {formatDuration(post.content.media?.[0].duration)}
            </div>
          )}
        </div>
      )
