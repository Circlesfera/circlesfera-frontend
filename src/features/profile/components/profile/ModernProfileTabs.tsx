...
import { getUserPosts } from '@/features/posts/services/postService';
...
export default function ModernProfileTabs({ username, isOwnProfile }: ModernProfileTabsProps) {
    ...
    try {
      ...
      switch (type) {
        case 'posts':
          ...
          const postsData = await getUserPosts(username, { pagination: { page, limit: 12 } });

          newContent = Array.isArray(postsData?.posts) ? postsData.posts : [];
          hasMore = (postsData?.pagination?.page || 0) < (postsData?.pagination?.pages || 0);
        ...
        case 'reels':
          ...
          newContent = Array.isArray(reelsData?.reels) ? reelsData.reels as unknown as (Post | Story | Reel)[] : [];
    ...
    if (error) {
      return (
        <div className="text-center py-16 px-6">
          ...
          <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3">Error al cargar contenido</h3>
  ...
  return (
    <div className="relative">
      ...
      <div className="relative bg-white dark:bg-gray-800 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100 dark:border-gray-700/50 overflow-hidden">
        {/* Pestañas mejoradas */}
        <div className="border-b border-gray-200 dark:border-gray-700 overflow-hidden rounded-t-xl">
          <div className="flex">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                ...
                className={`flex-1 flex items-center justify-center gap-2 px-4 py-4 text-sm font-medium transition-all duration-300 relative ${activeTab === tab.id
                  ? 'text-blue-600 dark:text-blue-400 bg-gradient-to-r from-blue-50/80 to-purple-50/80 dark:from-blue-900/30 dark:to-purple-900/30'
                  : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-700'
                  }`}
              >
                {activeTab === tab.id && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 to-purple-500"></div>
                )}
                <div className={`transition-colors duration-300 ${activeTab === tab.id ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>
                  {tab.icon}
                </div>
...
function ModernEmptyState({ tabType, isOwnProfile, router }: { tabType: TabType; isOwnProfile: boolean; router: AppRouterInstance }) {
  ...
  return (
    <div className="text-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-2">{message.title}</h3>
      <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-sm mx-auto leading-relaxed text-sm">{message.subtitle}</p>
      {isOwnProfile && (
          ...
          return (
            ...
            >
              <div className="absolute inset-0">
                <Image
                  src={story.content?.image?.url || '/placeholder.png'}
                  alt="Story"
                  fill
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3 lg:gap-4">
      {safeContent.map((item) => {
        ...
        const isCarousel = type === 'posts' && post.content.media && post.content.media.length > 1;
        ...
        return (
          <div
            ...
            className="aspect-[9/16] rounded-2xl lg:rounded-3xl overflow-hidden bg-gray-100 dark:bg-gray-700 relative group cursor-pointer shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
          >
            <div className="absolute inset-0">
              {type === 'posts' ? (
                <Image
                  src={post.content.media?.[0]?.url || '/placeholder.png'}
                  alt="Post"
                  fill
                  className="object-cover group-hover:scale-110 transition-transform duration-300"
              ...
              <>
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="w-12 h-12 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center">
                    <svg className="w-6 h-6 text-gray-900 dark:text-gray-100 ml-1" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3">
        {Array.from({ length: 8 }).map((_, index) => (
          <div key={index} className="aspect-square rounded-lg bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600 animate-pulse shadow-md" />
        ))}
      </div>
    </div>
  );
