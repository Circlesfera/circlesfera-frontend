          ...
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-4">
                <button
                  onClick={handleLike}
                  className={`flex items-center space-x-2 transition-colors ${reel.likes.some(like => like.user === currentUser?.id)
                    ? 'text-red-500'
                    : 'text-gray-600 dark:text-gray-400 dark:text-gray-500 hover:text-red-500'
                    }`}
                >
                  <svg className="w-6 h-6" fill={reel.likes.some(like => like.user === currentUser?.id) ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
